...
the named classes opt controls into the recipes, with no page-wide reset. The
kit reads the installed source at a new mount; retained frames keep the styling
they loaded. It introduces no theme polling, forced remount or mandatory visual
conformance. Author layout, validation and operations remain author-owned; the
small source recipes are in `docs/examples/author_ui_kit/`.

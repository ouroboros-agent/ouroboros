...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      │   ├── registry_core.py ← ToolRegistry execution authority: catalog loading, per-registry overlays, deterministic guard order, dispatch, and error projection
      ...
      │   ├── registry_guards.py ← Host-owned pre-dispatch task capability/resource, ephemeral, delegated-child, managed-update, skill-repair, and process root/CWD/workspace/runtime-secret/git guards; registry.py keeps only the proven compatibility re-exports
      │   ├── tool_resolution.py ← Physical root projections, dispatch path normalization, target-operation classification, and ordered call-scoped resource bindings; registry.py keeps exact compatibility re-exports

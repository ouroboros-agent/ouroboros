...
- `web/modules/composer_parts.js` is the ordered-composer-parts SSOT: the pure parts reducer (`pushText`/`pushChip`/`popLast`/`normalizeParts`) plus the ONE reversible `[context: <path> L<a>-L<b>]` marker codec (`serializeParts`/`parseContent`, fence length = longest backtick run + 1, selections over `MAX_CHIP_LINES` degrade to the bare whole-file marker, and a path that cannot round-trip is REFUSED at capture time rather than silently mangled), plus a thin `createComposerParts({container, input})` DOM mount over the same core. It carries no transport and no send logic — the serialized marker string is ordinary message text, so `contracts.py` and the system prompt are untouched. Every composer (chat, and the Changes / Files docks) mounts this module and hands its ordered parts to the chat controller's `setDraftParts` / `sendParts`.
...
`web/modules/changes.js` (`#page-changes`) is the diff-review screen: a task rail from `GET /api/tasks` (newest first; the project badge is the task's OWN `project_id` first and the `/api/state.task_bindings` post-hoc binding second, names from the already-fetched project summaries, falling back to the raw id so a project beyond the capped sidebar list is still labelled), a file list, and unified / split renderers over the same parsed hunks. `web/modules/task_inspector.js` registers the `inspector` right-panel kind against the app-level state machine (mutually exclusive with the project panel, fixed `--inspector-width`, mobile overlay), opens on the `ouro:inspect-task` event a live card dispatches, and carries exactly two tabs — Changes (file rows → the full Changes screen) and Cost — plus a footer of `+N −N`, task cost and elapsed. Its Cost tab renders ONLY fields the task record actually persists (accounted cost with its accounting status/final flag, rounds, prompt/completion tokens, reserved, unresolved upper bound, unknown-unmetered) under the same fail-closed discipline as the chat card's `taskCostMeta`: an unreadable ledger reads "Unavailable", never a convincing `$0.00`. Elapsed reads the durable `duration_sec` scalar `agent_task_pipeline._store_task_result` persists — the SAME value the `task_metrics` event already carries (one timing, two consumers) — so a task whose live event stream the browser never saw still shows an honest elapsed instead of a fabricated zero. The inspector's live refresh rides the ONE app-owned `/api/state` poll and stops once the task is terminal.

Diff PARSING is client-side (`web/modules/patch_parse.js`, pure and node-tested): the server returns the raw patch and the browser derives the file list, per-file status (M/A/D/rename/binary) and +/− counts from those exact bytes. One snapshot is one truth — a second server-side stat source could silently disagree with the patch the owner is reading. The parser closes a hunk on the line counts the hunk header itself declared (not on "this line looks like a header"), which is what keeps a DELETED line whose content starts with `--- ` a deletion; split view pairs each deletion run with the following addition run and renders an empty counterpart cell for the shorter side. Nothing is smoothed over: `pending`, `blocked` (with its typed blocker tokens) and HEAD drift each get an explicit banner, and the drift sentence is a boolean disclosure — "HEAD differs from the task baseline; showing the current projection for paths attributed during the task window" — carrying no commit counts and no exclusive-ownership claim (attribution is evidence, not exclusion). The screen's only action is "Request edits": the dock is a `composer_parts` instance whose ORDERED parts go to the chat controller's `sendParts` behind a prepended plain-text `Re task <id> ("<title>"): ` line, and the dock clears only after that handoff succeeds. There is deliberately no Approve control (owner-locked scope).
...
| GET | `/api/tasks/{task_id}/diff` | `gateway.tasks.api_task_diff` (typed diff projection for the Changes screen / task inspector — see below) |
...
- `GET /api/tasks/{task_id}/diff` answers ONE typed diff projection `{status, source, base_commit, head_advanced, blockers[], patch, patch_sha256}` for the Changes screen and the task inspector. The whole handler runs off the event loop (`asyncio.to_thread`): it reads artifact bytes and shells out to git. `status` is `pending | ready | empty | blocked` derived from the effective task status plus `artifact_status`, and the ONLY 404 is an unknown task id — a running workspace task whose artifacts are not finalized is `pending`, never a 404 and never a fabricated "no changes". A WORKSPACE task's `source` is `workspace_patch`: the patch is the durable `workspace.patch` artifact's bytes, with `workspace_patch.json` supplying `base_commit`/HEAD drift and its `errors[]` as blockers (`ready_no_changes` → `empty`, `failed` → `blocked`). Artifact lookup, name matching and directory containment live in ONE shared private resolver (`_resolve_task_artifact_path`) used by both this endpoint and `api_task_artifact`, so the traversal guard exists once; a declared-but-escaping path is refused as a typed blocker rather than served. A SELF-REPO task has no historical patch, so `source` is `mutation_baseline`: an honestly-labelled LIVE projection whose candidate set comes from the persisted `terminal_candidate_snapshot` for a terminal task and from `attributed_git_candidates()` while the task runs (the mutation-attribution SSOT — this endpoint never re-parses raw evidence), matched to the ONE `system_repo` baseline surface whose canonical root equals the request's repo dir. The patch is `git diff <base_commit> -- <candidates>` plus a `--no-index` section per attributed untracked file, with `--no-ext-diff --no-textconv` so a repo-configured external differ can never execute on the host while answering a browser read (BIBLE P3). The read is BOUND to the state it was computed from: HEAD plus a per-candidate stat fingerprint is taken before and after, a mismatch retries ONCE, and a second mismatch answers `blocked` with `projection_changed_during_read` rather than a patch that does not belong to the disclosed baseline. Attribution blockers are passed through untouched, and `baseline_stale` — the drift condition itself — is surfaced as the boolean `head_advanced` while the current projection is still shown, because refusing there would hide real work behind an evidence footnote. Consequence worth knowing: a RUNNING self-repo task that already COMMITTED all its work has no live candidate set (the live authority attributes working-tree-dirty paths only), so it answers `blocked` + `baseline_stale` until it terminalizes and its snapshot — which does combine dirty and committed paths — is recorded. There is deliberately NO server-side file-stat block and NO patch truncation: the client parses the same bytes it renders (one snapshot, one truth), and an owner-facing diff is never silently clipped.

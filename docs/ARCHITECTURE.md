...
| OUROBOROS_TASK_DIFF_GIT_TIMEOUT_SEC | 30 | Per-invocation timeout (seconds) for the git subprocesses behind `GET /api/tasks/{task_id}/diff` (`gateway/task_diff.py::_git_capture`); clamped 5-300 via the shared `_clamped_setting` reader in `config.py`. |

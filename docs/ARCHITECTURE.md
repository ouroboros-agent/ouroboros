...
Chart.js is bundled locally as `web/chart.umd.min.js`; no CDN dependency by design. Because a canvas cannot resolve CSS `var(...)`, `web/modules/utils.js::readThemeTokens` is the single canvas↔token boundary: the chart-theme helpers `evolution.js::evolutionChartTheme` and `widgets.js::chartPalette` (with `evolution.js::chartColorAlpha` for the dataset fills) resolve every series colour, axis colour and font stack off the live `:root` computed style at chart-build time, so no chart holds a hand-synced copy of the design tokens — and the token names they pass must be LEAF values, since an alias would come back as the literal string `var(...)` (pinned by `tests/test_widgets_ui_static.py::test_canvas_palettes_read_only_leaf_root_tokens`).

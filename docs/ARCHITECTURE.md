...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      │   ├── tool_result.py   ← Internal ToolResult/ToolCodeSpec owner, finite code table, byte-compatible legacy text adapter, and registry composer facade
      ...
      │   ├── registry_core.py ← ToolRegistry orchestration authority: catalog loading, per-registry overlays, exact guard order, builtin invocation, and compatibility result projection
      │   ├── registry_guard_process.py ← Pre-execution process/shell coordinator plus post-execution owner-state restore, light-repo, and workspace-ref tripwires; invokes root/CWD/workspace/git decisions directly through registry_guards

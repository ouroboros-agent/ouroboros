...
The dock is also a `[data-capture-dock]`, so ⌘L works here exactly as on Files: the global handler (`app.js [anchor:phase-C]`) names the active page in one CANCELABLE `ouro:capture-selection` event, and this page consuming it — `preventDefault()` — is what licenses suppressing the browser default. A diff selection becomes a chip named by its NEW-side line numbers with the selected diff lines inlined verbatim, because those are the lines that exist in the file the agent will edit; a selection whose boundary is a DELETION has no new-side number to name, so it omits the range (and with it, per the codec, the bytes) and degrades to the whole-file marker rather than pointing at lines that are gone. No selection is the whole-file chip. The always-visible "Add selection ⌘L" button is the GUARANTEED path — some browsers keep ⌘L for the address bar — and suppresses its own mousedown default so it cannot destroy the selection it is about to read.
